/**
 * HiveGate — MPP (Machine Payments Protocol) Middleware
 *
 ...
 *
 * Stream E — admission, pricing tiers, identity
...
const PAYMENT_ADDRESS = (
  process.env.HIVEGATE_PAYMENT_ADDRESS ||
...
// ─── Fee table ───────────────────────────────────────────────
// Stream E: admission, pricing tiers

const DID_CREDENTIAL_PRICING = {
  '/v1/gate/onboard':          1.00,
  '/v1/gate/admit':            0.10,
  '/v1/gate/tier/upgrade':     1.00,
  '/v1/gate/tier/verify':      0.10,
  '/v1/gate/register-guest':   4.99,
};

function getMppPrice(path) {
  if (DID_CREDENTIAL_PRICING[path]) return DID_CREDENTIAL_PRICING[path];
  if (path.startsWith('/v1/gate/onboard')) return 1.00;
  if (path.startsWith('/v1/gate/admit'))   return 0.10;
  if (path.startsWith('/v1/gate/tier'))    return 0.10;
  if (path.startsWith('/v1/gate/'))        return 0.10;
  return 0.10;
}
